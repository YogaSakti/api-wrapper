...
router.get('/', function (req, res) {
    const endpoints = [
        ...
        'ottr',
        'awag'
    ].map(endpoint => `<a href="/${endpoint}">${endpoint}</a>`).join('<br/>')
...
router.get('/collected/:address', asyncHandler(async (req, res, next) => {
    const address = req.params.address
    const nfts = await getOwned(address)
    // Array.from(new Map(nfts.collectibles.map((obj: { collection: { name: any; }; }) => [obj.collection.name, obj])).values()).forEach((nft: any) => {
    //     console.log(nft.collection.name);
    // })

    const nftData = nfts.collectibles
    const countByColl = (collectionName: string, equalityCheck = false) => nftData.filter((nft: { collection: { name: string | string[]; }; }) => {
        if (equalityCheck) return nft.collection.name === collectionName
        return nft.collection.name?.includes(collectionName)
    }).length

    // nftData = nftData.filter((nft: { collection: { name: string | string[]; }; }) => !nft.collection.name.includes('Compass Rose'))

    const formatedData = {
        seasonOne: countByColl('DRiP', true),
        seasonTwo: countByColl('#2'),
        degen: countByColl('Degen'),
        daa: countByColl('DAA'),
        vault: countByColl('Vault'),
        floor: countByColl('Floor'),
        tiiiny: countByColl('Tiiiny'),
        faceless: countByColl('Faceless'),
        bangerz: countByColl('Bangerz'),
        betdex: countByColl('BetDEX'),
        geneftee: countByColl('GENEFTEE'),
        maquin: countByColl('Maquin'),
        andrewmason: countByColl('Stolen Rainbows')
    }
...
router.get('/owned/:address', asyncHandler(async (req, res, next) => {
    const address = req.params.address
    const nfts = await getOwned(address)
    let owned = nfts.collectibles.filter((nft) => nft.owner === address)
    owned = owned.map((nft: { name: any; attributes: any[]; }) => {
        let nftName = nft.name
        let rarity = nft.attributes.find((z) => z.trait_type === 'Rarity')?.value || ''
        rarity = rarity.charAt(0).toLowerCase()

        if (nftName.includes('Faceless')) {
            nftName = nftName.replace('Faceless', `Faceless (${rarity.charAt(0).toLowerCase()})`).split(' #')[0]
        } else if (nftName.includes('SEC')) {
            nftName = `${nftName.trim()} (${rarity.charAt(0).toLowerCase()})`
        } else if (nftName.includes('Chapter Three') || nftName.includes('Chapter Four')) {
            nftName = `${nftName.trim()} (${rarity.charAt(0).toLowerCase()})`
        } else if (nftName.includes('King Robert')) {
            nftName = `${nftName.trim()} (${rarity.charAt(0).toLowerCase()})`
        } else if (nftName.includes('Arginine')) {
            nftName = `${nftName.trim()} (${rarity.charAt(0).toLowerCase()})`
        } else if (nftName.includes('Pierspective')) {
            nftName = `${nftName.trim()} (${rarity.charAt(0).toLowerCase()})`
        }

        if (nftName.includes('Compass Rose')) nftName = nftName.split(' #')[0]
        if (nftName.includes('’')) nftName = nftName.replace('’', '\'')

        return nftName
    })

    owned = owned.sort((a: string, b: string) => a.localeCompare(b))
    ...
    res.status(200).send({ nft: owned.join(';') })
}))

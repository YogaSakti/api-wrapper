 ...
 */
router.get('/dex/:platform/:address', async (req, res) => {
    if (!isSafePlatform(req.params.platform) || !isSafeAddress(req.params.address)) {
        return res.status(400).json({
            error: 'Invalid Parameters',
            message: 'Platform or address contains unsupported characters or is too long'
        })
    }
...
router.get('/:slug', async (req, res) => {
    ...
    if (!isSafeSlug(req.params.slug) || !isFiatCode(fiat.toUpperCase())) {
        return res.status(400).json({
            error: 'Invalid Parameters',
            message: 'Slug or convert parameter is invalid'
        })
    }

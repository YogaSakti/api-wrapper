...
const getSpotBalance = async () =>
    client.getAllCoinsBalance({ accountType: 'FUND' }).then((response: any) => {
        if (response.retCode !== 0) throw new Error(`Error fetching balances: ${response.retMsg}`)
        const balances = response.result?.balance
        return Array.isArray(balances) ? balances.filter((balance: BybitCoinBalance) => toAmount(balance.walletBalance) !== 0) : []
    })

const getEarnPositions = async (category: BybitEarnCategory = BYBIT_EARN_CATEGORY.FLEXIBLE) =>
    client.getEarnPosition({ category }).then((response: any) => {
...
const getUnifiedBalance = async () =>
    client.getAllCoinsBalance({ accountType: 'UNIFIED', coin: BYBIT_COIN.BYUSDT }).then((response: any) => {
        if (response.retCode !== 0) throw new Error(`Error fetching unified balance: ${response.retMsg}`)
        const balances = response.result?.balance
        return Array.isArray(balances) ? balances.filter((balance: BybitCoinBalance) => toAmount(balance.walletBalance) !== 0) : []
    })

const groupByCoin = <T extends { coin: string }>(items: T[]): Record<string, T[]> =>
    items.reduce(
        (groups, item) => {
...
const firstAmountByCoin = <T extends { walletBalance?: string; amount?: string }>(groups: Record<string, T[]>, coin: string, field: keyof T): number => toAmount(groups[coin]?.[0]?.[field])

const calculateOnChainAmount = (positions: BybitEarnPosition[]): number => positions.reduce((total, position) => total + toAmount(position.amount) + toAmount(position.totalPnl), 0)
...
const buildBybitBalanceMap = ({ spotBalances, earnPositions, onChainBalances, unifiedBalances }: BybitBalanceSources): NumericBalanceMap => {

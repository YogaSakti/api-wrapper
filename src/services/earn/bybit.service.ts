...
export const data_Bybit = async (): Promise<EarnAprItem[]> => {
    try {
        const response = await fetch('https://api2.bybit.com/s1/byfi/get-saving-homepage-product-cards', {
            headers: {
                ...BYBIT_HEADERS,
        ...
        const filtered = json.result.coin_products.filter((i: any) => i.coin === 5 || i.coin === 16)

        return filtered.map((item: any) => {
            const savingProd = item.saving_products.find((x: { coin: number }) => x.coin === item.coin)
...
interface BybitAirdropConfig {
    coin: number
    name: string
    referer: string
}

const dataBybitAirdrop = async ({ coin, name, referer }: BybitAirdropConfig): Promise<EarnAprItem[]> => {
    try {
